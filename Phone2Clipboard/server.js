...
/**
 * 写入 Windows 系统剪贴板
 * 通过临时文件传递文本，避免命令行参数转义问题
 * 支持任意字符（单引号、双引号、特殊符号、emoji 等）
 */
function setClipboard(text) {
  const tmpFile = path.join(os.tmpdir(), "clip_sync_tmp.txt");
  // 写入临时文件（UTF-8 编码）
  fs.writeFileSync(tmpFile, text, "utf-8");
  try {
    // 用 cmd /c 包裹，避免 PowerShell 参数转义问题
    const cmd = `powershell -NoProfile -Command "$t = [System.IO.File]::ReadAllText('${tmpFile.replace(/\\/g, "\\\\")}', [System.Text.Encoding]::UTF8); Set-Clipboard -Value $t"`;
